...
PEER0_BENEFICIARY_CA=/opt/gopath/src/github.com/hyperledger/fabric/peer/crypto/peerOrganizations/beneficiary.property-registration-network.com/peers/peer0.beneficiary.property-registration-network.com/tls/ca.crt
PEER0_AGENT_CA=/opt/gopath/src/github.com/hyperledger/fabric/peer/crypto/peerOrganizations/agent.property-registration-network.com/peers/peer0.agent.property-registration-network.com/tls/ca.crt
PEER0_GOVERNMENT_CA=/opt/gopath/src/github.com/hyperledger/fabric/peer/crypto/peerOrganizations/government.property-registration-network.com/peers/peer0.government.property-registration-network.com/tls/ca.crt
PEER0_BUYER_CA=/opt/gopath/src/github.com/hyperledger/fabric/peer/crypto/peerOrganizations/buyer.property-registration-network.com/peers/peer0.buyer.property-registration-network.com/tls/ca.crt
...
setGlobals() {
  PEER=$1
  ORG=$2
  if [ "$ORG" == 'beneficiary' ]; then
    CORE_PEER_LOCALMSPID="beneficiaryMSP"
    CORE_PEER_TLS_ROOTCERT_FILE=$PEER0_BENEFICIARY_CA
    ...
    CORE_PEER_TLS_ROOTCERT_FILE=$PEER0_AGENT_CA
    CORE_PEER_MSPCONFIGPATH=/opt/gopath/src/github.com/hyperledger/fabric/peer/crypto/peerOrganizations/agent.property-registration-network.com/users/Admin@agent.property-registration-network.com/msp
    if [ "$PEER" -eq 0 ]; then
    ...
    CORE_PEER_MSPCONFIGPATH=/opt/gopath/src/github.com/hyperledger/fabric/peer/crypto/peerOrganizations/government.property-registration-network.com/users/Admin@government.property-registration-network.com/msp
    if [ "$PEER" -eq 0 ]; then
    ...
    CORE_PEER_MSPCONFIGPATH=/opt/gopath/src/github.com/hyperledger/fabric/peer/crypto/peerOrganizations/buyer.property-registration-network.com/users/Admin@buyer.property-registration-network.com/msp
    if [ "$PEER" -eq 0 ]; then
...
instantiateChaincode() {
  ...
  if [ -z "$CORE_PEER_TLS_ENABLED" -o "$CORE_PEER_TLS_ENABLED" = "false" ]; then
    set -x
    peer chaincode instantiate -o orderer.property-registration-network.com:7050 -C "$CHANNEL_NAME" -n regnet -l "${LANGUAGE}" -v "${VERSION}" -c '{"Args":["org.property-registration-network.regnet.agent:instantiate","org.property-registration-network.regnet.government:instantiate","org.property-registration-network.regnet.buyer:instantiate","org.property-registration-network.regnet.beneficiary:instantiate"]}' -P "OR ('beneficiaryMSP.member','agentMSP.member','governmentMSP.member','buyerMSP.member')" >&log.txt
    res=$?
    set +x
  else
    set -x
    peer chaincode instantiate -o orderer.property-registration-network.com:7050 --tls "$CORE_PEER_TLS_ENABLED" --cafile $ORDERER_CA -C $CHANNEL_NAME -n regnet -l ${LANGUAGE} -v ${VERSION} -c '{"Args":["org.property-registration-network.regnet:instantiate"]}' -P "OR ('beneficiaryMSP.member','agentMSP.member','governmentMSP.member','buyerMSP.member')" >&log.txt
    res=$?
    set +x
  fi
  cat log.txt
...
upgradeChaincode() {
  ...
  if [ -z "$CORE_PEER_TLS_ENABLED" -o "$CORE_PEER_TLS_ENABLED" = "false" ]; then
    ...
    set -x
    peer chaincode upgrade -o orderer.property-registration-network.com:7050 --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA -C $CHANNEL_NAME -n regnet -l ${LANGUAGE} -v ${VERSION} -p ${CC_SRC_PATH} -c '{"Args":["org.property-registration-network.regnet:instantiate"]}' -P "OR ('beneficiaryMSP.member','usersMSP.member')" >&log.txt
    res=$?
    set +x
  fi
  cat log.txt
...
chaincodeInvoke() {
  ...
  if [ -z "$CORE_PEER_TLS_ENABLED" -o "$CORE_PEER_TLS_ENABLED" = "false" ]; then
    set -x
    peer chaincode invoke -o orderer.property-registration-network.com:7050 -C $CHANNEL_NAME -n regnet $PEER_CONN_PARMS -c '{"Args":["org.property-registration-network.regnet:createStudent","0001","Chandrika Basak","connect@chandrikab.com","15"]}' >&log.txt
    res=$?
    set +x
  else
    set -x
    peer chaincode invoke -o orderer.property-registration-network.com:7050 --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA -C $CHANNEL_NAME -n regnet $PEER_CONN_PARMS -c '{"Args":["org.property-registration-network.regnet:createStudent","0001","Chandrika Basak","connect@chandrikab.com"]}' >&log.txt
    res=$?
    set +x
  fi
  cat log.txt
