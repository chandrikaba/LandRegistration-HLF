...
function replacePrivateKey() {
  ...
  CURRENT_DIR=$PWD
  cd crypto-config/peerOrganizations/beneficiary.property-registration-network.com/ca/ || exit
  PRIV_KEY=$(ls *_sk)
  cd "$CURRENT_DIR" || exit
  sed $OPTS "s/BENEFICIARY_CA_PRIVATE_KEY/${PRIV_KEY}/g" docker-compose-e2e.yml
  cd crypto-config/peerOrganizations/agent.property-registration-network.com/ca/ || exit
  PRIV_KEY=$(ls *_sk)
  cd "$CURRENT_DIR" || exit
  sed $OPTS "s/AGENT_CA_PRIVATE_KEY/${PRIV_KEY}/g" docker-compose-e2e.yml
  cd crypto-config/peerOrganizations/government.property-registration-network.com/ca/ || exit
  PRIV_KEY=$(ls *_sk)
  cd "$CURRENT_DIR" || exit
  sed $OPTS "s/GOVERNMENT_CA_PRIVATE_KEY/${PRIV_KEY}/g" docker-compose-e2e.yml
  cd crypto-config/peerOrganizations/buyer.property-registration-network.com/ca/ || exit
  PRIV_KEY=$(ls *_sk)
  cd "$CURRENT_DIR" || exit
  sed $OPTS "s/BUYER_CA_PRIVATE_KEY/${PRIV_KEY}/g" docker-compose-e2e.yml
...
function generateChannelArtifacts() {
  ...
  echo "#################################################################"
  set -x
  configtxgen -profile RegistrationChannel -outputAnchorPeersUpdate ./channel-artifacts/beneficiaryMSPanchors.tx -channelID "$CHANNEL_NAME" -asOrg beneficiaryMSP
  res=$?
  set +x
  if [ $res -ne 0 ]; then
    echo "Failed to generate anchor peer update for beneficiary..."
    exit 1
  fi
  ...
  echo "#################################################################"
  set -x
  configtxgen -profile RegistrationChannel -outputAnchorPeersUpdate ./channel-artifacts/agentMSPanchors.tx -channelID "$CHANNEL_NAME" -asOrg agentMSP
  res=$?
  set +x
  if [ $res -ne 0 ]; then
    echo "Failed to generate anchor peer update for agent..."
    exit 1
  fi
  echo
  ...
  echo "#################################################################"
  set -x
  configtxgen -profile RegistrationChannel -outputAnchorPeersUpdate ./channel-artifacts/governmentMSPanchors.tx -channelID "$CHANNEL_NAME" -asOrg governmentMSP
  res=$?
  set +x
  if [ $res -ne 0 ]; then
  ...
  echo
  echo "#################################################################"
  echo "#######    Generating buyer peer update for governmentMSP   ##########"
  echo "#################################################################"
  set -x
  configtxgen -profile RegistrationChannel -outputAnchorPeersUpdate ./channel-artifacts/buyerMSPanchors.tx -channelID "$CHANNEL_NAME" -asOrg buyerMSP
  res=$?
  set +x
  if [ $res -ne 0 ]; then
    echo "Failed to generate anchor peer update for buyer..."
    exit 1
  fi
  echo
